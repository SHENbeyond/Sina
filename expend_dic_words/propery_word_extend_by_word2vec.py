...
path_property = open("car_entity_property.txt",'r')
model_w2v = gensim.models.Word2Vec.load_word2vec_format("vectors_test.bin", binary=True)
path_out = "words_propery_list.txt"
path_out_not = "words_propery_list_not_in.txt"
...
if __name__ == '__main__':
    property_set = fun_propertyset(path_property)
    words_list = word_by_score(property_set,model_w2v)
    for i in words_list:
        if not i[0].encode('utf-8') in property_set:
            print i[0],type(i[0])
            print i[1],type(i[1])
